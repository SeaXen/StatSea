...
@router.websocket("/ws/speedtest")
async def speedtest_websocket(websocket: WebSocket, provider: str = "ookla"):
    await websocket.accept()
    try:
        # Create a queue to communicate between the speedtest thread and the websocket loop
        queue = asyncio.Queue()

        def progress_callback(data):
            # This runs in the speedtest thread
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            try:
                # We need to put data into the queue in a thread-safe way
                # Since queue.put is async, and we are in a sync function/thread
                # It's better to use run_coroutine_threadsafe if we had the main loop reference.
                # However, simplistic approach: passing the main loop to the service isn't ideal.
                # BETTER APPROACH:
                # speedtest_service.run_speedtest runs in wait_for_executor.
                # The callback specific logic needs to bridge the gap.
                pass
            except Exception as e:
                logger.error(f"Callback error: {e}")

        # RE-THINKING ARCHITECTURE for Thread-Safety:
        # We need the main event loop to use run_coroutine_threadsafe.
        ...
        # Start the speedtest in a background task so we can also listen for queue
        # But wait... run_speedtest awaits the executor.
        
        # We need to run run_speedtest concurrently with a loop that consumes the queue.
