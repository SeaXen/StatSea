...
def check_device_availability():
    ...
    try:
        ...
        offline_candidates = db.query(Device).filter(
        ...
        for device in offline_candidates:
            logger.info(f"Device {device.hostname or device.mac_address} ({device.ip_address}) is now OFFLINE. Last seen: {device.last_seen}")
