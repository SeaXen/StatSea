...
from ..models.models import SystemDailySummary, DeviceDailySummary, Device
...
class AIPredictor:
    ...
    def predict_total_usage(self) -> Dict:
        ...
        try:
            ...
            summaries = db.query(SystemDailySummary).order_by(desc(SystemDailySummary.date)).limit(30).all()
            if len(summaries) < 3:
            ...
            return {
                ...
                "trend": trend,
                "growth_rate_pct": float(model.coef_[0] / np.mean(y) * 100) if np.mean(y) > 0 else 0
            }
    ...
    def detect_anomalies(self) -> List[Dict]:
        ...
        try:
            ...
            for dev in devices:
                ...
                ).order_by(desc(DeviceDailySummary.date)).limit(14).all()
