...
from ..models.models import (
    SystemNetworkHistory, SystemDailySummary, SystemMonthlySummary, SystemYearlySummary,
    TrafficLog, DeviceDailySummary, DeviceMonthlySummary, DeviceYearlySummary, Device
)
    ...
    try:
        ...
        for day_offset in [0, 1]:
            ...
            stats = db.query(
                ...
                func.date(SystemNetworkHistory.timestamp) == target_date
            ).first()

            if stats and (stats.bytes_sent or stats.bytes_recv):
                summary = db.query(SystemDailySummary).filter(
                    SystemDailySummary.date == target_date
                ).first()
...
def aggregate_system_monthly(db_session=None):
    ...
    try:
        ...
        stats = db.query(
            ...
            func.strftime("%Y-%m", SystemDailySummary.date) == month_str
        ).first()

        if stats and (stats.bytes_sent or stats.bytes_recv):
            summary = db.query(SystemMonthlySummary).filter(
                SystemMonthlySummary.month == month_str
            ).first()
            if not summary:
    ...
    try:
        ...
        for dev in devices:
            ...
            if stats and (stats.upload or stats.download):
                summary = db.query(DeviceMonthlySummary).filter(
...
def aggregate_system_yearly(db_session=None):
    ...
    try:
        ...
        stats = db.query(
            ...
            func.cast(func.substr(SystemMonthlySummary.month, 1, 4), Integer) == year
        ).first()

        if stats and (stats.bytes_sent or stats.bytes_recv):
            summary = db.query(SystemYearlySummary).filter(
                SystemYearlySummary.year == year
            ).first()
            if not summary:
...
def aggregate_device_yearly(db_session=None):
    ...
    try:
        ...
        for dev in devices:
            ...
            if stats and (stats.upload or stats.download):
                summary = db.query(DeviceYearlySummary).filter(
                    DeviceYearlySummary.device_id == dev.id,
                    DeviceYearlySummary.year == year
                ).first()
                if not summary:
