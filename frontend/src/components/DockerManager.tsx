                                ...
                                <div className="flex flex-col lg:flex-row lg:items-center gap-6 relative z-10">
                                    ...
                                    <div className="flex items-start gap-4 lg:w-1/4">
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <p className="text-xs text-gray-500 font-mono truncate opacity-60">IMAGE: {container.image}</p>
                                            <div className="flex items-center gap-2 mt-2">
                                                ...
                                                <span className="text-[10px] text-gray-600 font-mono">ID: {container.id.substring(0, 12)}</span>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        >
                            ...
                            <div className="sticky top-0 z-20 bg-gray-950/80 backdrop-blur-md border-b border-gray-800 p-6 flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <div className={`p-3 rounded-xl border ${getStatusColor(selectedContainer.status)}`}>
                                        <Container className="w-6 h-6" />
                                    </div>
                                    <div>
                                        <h2 className="text-xl font-bold text-white tracking-tight uppercase">{selectedContainer.name}</h2>
                                        <p className="text-xs text-gray-500 font-mono opacity-60">ID: {selectedContainer.id}</p>
                                    </div>
                                </div>
                            ...
                            <div className="p-6 space-y-8 flex-1">
                                ...
                                <div className="flex items-center gap-3 p-4 bg-gray-900/50 rounded-2xl border border-gray-800/50">
                                    ...
                                    >
                                        {actionLoading === `${selectedContainer.id}-start` ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4" />}
                                        Start
                                    </button>
                                    <button 
                                        disabled={!!actionLoading}
                                    ...
                                    >
                                        {actionLoading === `${selectedContainer.id}-stop` ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Square className="w-4 h-4" />}
                                        Stop
                                    </button>
                                    <button 
                                        disabled={!!actionLoading}
                                    ...
                                    >
                                        {actionLoading === `${selectedContainer.id}-restart` ? <RefreshCw className="w-5 h-5 animate-spin" /> : <RotateCcw className="w-5 h-5" />}
                                    </button>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-gray-900/40 border border-gray-800/50 rounded-2xl p-4">
                                        ...
                                        <div className="text-3xl font-mono font-bold text-white mb-2">{selectedContainer.cpu_pct.toFixed(2)}%</div>
                                        <div className="h-32 w-full">
                                            <ResponsiveContainer width="100%" height="100%">
                                                <AreaChart data={selectedContainer.history.cpu.map(val => ({ val }))}>
                                    ...
                                    <div className="bg-gray-900/40 border border-gray-800/50 rounded-2xl p-4">
                                        ...
                                        <div className="text-3xl font-mono font-bold text-white mb-2">{selectedContainer.mem_usage}MB</div>
                                        <div className="h-32 w-full">
                                            <ResponsiveContainer width="100%" height="100%">
                                                <AreaChart data={selectedContainer.history.mem.map(val => ({ val }))}>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        {[
                                            { label: 'Status', value: selectedContainer.status, icon: Activity, color: getStatusColor(selectedContainer.status).split(' ')[0] },
                                            { label: 'Uptime', value: selectedContainer.state?.StartedAt ? new Date(selectedContainer.state.StartedAt).toLocaleString() : 'N/A', icon: Clock },
                                            { label: 'Exit Code', value: selectedContainer.state?.ExitCode ?? 'N/A', icon: Shield },
                                            { label: 'Image Tag', value: selectedContainer.image.split(':')[1] || 'latest', icon: Container }
                                        ].map((item, i) => (
                                            <div key={i} className="flex items-center gap-3 p-3 bg-gray-900/40 rounded-xl border border-gray-800/30">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="text-xs font-mono text-white mt-0.5 truncate max-w-[150px]">{item.value.toString()}</div>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-8">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-lg font-mono font-bold text-white">{formatBytes(selectedContainer.net_rx)}</div>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-lg font-mono font-bold text-white">{formatBytes(selectedContainer.net_tx)}</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="p-4 bg-gray-900/40 rounded-2xl border border-gray-800/50">
                                        ...
                                        <div className="space-y-2">
                                            {selectedContainer.ports && Object.keys(selectedContainer.ports).length > 0 ? (
                                                Object.entries(selectedContainer.ports).map(([p, b]: any) => (
                                    ...
                                    <div className="p-4 bg-gray-900/40 rounded-2xl border border-gray-800/50">
                                        ...
                                        <div className="space-y-2">
                                            {selectedContainer.mounts && selectedContainer.mounts.length > 0 ? (
                                                selectedContainer.mounts.map((m, i) => (
                                                    <div key={i} className="text-[10px] bg-black/30 p-2 rounded-lg border border-gray-800/30">
                                                        <div className="text-gray-500 uppercase mb-1">{m.Type}</div>
                                                        <div className="text-white font-mono break-all">{m.Source || m.Name}</div>
